...
char sync_data[SYNC_SIZE] = {	0xFF, 0x50, 0x51, 0x52, 0x53, 0x54, 0x55, 0x56, 0x57,
       ...
       // printf("%u[%d] ",rx_buff[bit_count],bit_count);
		...
		if (bit_count == 8) {
          ...
          //  printf("%x ",payload_data[data_len]);
	       // printf("%x ",convert_8_bits_to_char(buffer));
            data_len += 1;
			bit_count = 0;
		}
	}
...
void check_confidence_and_ret_payload(packet_s* P1,int length)
{
  ...
  //  scanf("%d",&confidence_level);
    ...
    if(calculated_confidence >= confidence_level)
    {
        ...
        detect_and_print_payload(payload_position+1,payload_data); //Find the payload of the data
    }
    else
    {
